...
class Preprocess(Action):
    ...
    def run(self):
        ...
        if ret:
            ...
            msg = 'filtering diffing %r, high pass freq %r Hz, low pass freq %r Hz' % (diff, lo, hi)
            ...
            ds.preprocess = ('Diff' if diff else 'No diff') + ', band pass between %.2f and %.2f Hz' % (lo, hi)
...
class ICA(Action):
    ...
    def run(self):
        cfg = ParamsDialog(parent=self.main, parameters=[
            ...
            {'name': 'Algorithm', 'type': 'list', 'values': ['parallel', 'deflation'], 'value': 'parallel'},
            {'name': 'Neg-entropy approx function', 'type': 'list', 'values': ['logcosh', 'exp', 'cube'], 'value': 'logcosh'},
            {'name': 'Max iterations', 'type': 'int', 'value': 200, 'limits': (1, 1000000), 'step': 20},
        ...
        if cfg.exec_():
            ...
            if ncomp < 1:
                ncomp = None
            ds.ica = FastICA(n_components=ncomp, algorithm=algo, fun=negent, max_iter=maxiter, tol=tol)
...
class PickChannels(Action):
    ...
    def run(self):
        ...
        for p in ds.picks:
            cs.cbs[p].setCheckState(qt_core_enum('CheckState', 'Checked')) # QtCore.Qt.CheckState.Checked)
        ret = cs.exec_()
        if ret:
...
class Decimate(Action):
    ...
    def run(self):
        cfg = ParamsDialog(parent=self.main, parameters=[
            {'name': 'Decimation factor', 'type': 'int',
                'value': 5, 'limits': (1, 100)}])
        if cfg.exec_():
            ds = self.main.dataset
            factor, = cfg.values
            ds.fs /= factor
...
actions = [Preprocess, Decimate, SpatialWhiten, ICA, Bipolarize, PickChannels, 'separator', MergeEvents, 
       RemoveSelectedEvents, RejectEvents, ClearEvents]
