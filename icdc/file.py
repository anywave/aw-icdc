...
class LoadVHDR(Action):
    ...
    def run(self, filename=None):
        filename = filename or find_file('BrainVision headers (*.vhdr)', parent=self.main)
        if filename:
            self.status.emit('reading %s' % (filename, ))
...
class LoadAWMarkers(Action):
    ...
    def run(self, filename=None):
        filename = filename or find_file(filt='AnyWave marker files (*.mrk)', parent=self.main)
        if filename:
            ...
            with open(filename, 'r') as fd:
                for line in fd.readlines():
                    ...
                    ds.events.append({
                        ...
                        'targets': [t for t in parts[4].strip().split(',') if len(t) > 0]
                    })
...
class SaveAWMarkers(Action):
    ...
    def run(self):
        filename = find_file(filt='AnyWave marker files (*.mrk)', 
                             parent=self.main, mode='save')
        if filename:
            ds = self.main.dataset
            ...
            with open(filename, 'w') as fd:
                ...
                for e in ds.events:
                    ...
                    line = '%s\t%d\t%.3f\t%f\t%s\n'
                    if targets and (type(targets[0]) not in (str, unicode)):
...
class SaveEventsMAT(Action):
    ...
    def run(self, filename=None):
        filename = filename or find_file(filt='MATLAB file (*.mat)',
                                         parent=self.main, mode='save')
        if filename:
...
class SaveWorkspace(Action):
    ...
    _shortcut = "Ctrl+S"
    def run(self, filename=None):
        filename = filename or find_file(filt='Workspace pickles (*.pickle)',
                                         parent=self.main, mode='save')
        if filename:
...
class LoadWorkspace(Action):
    ...
    _shortcut = "Ctrl+O"
    def run(self, filename=None):
        filename = filename or find_file(filt='Workspace pickles (*.pickle)', parent=self.main)
        if filename:
...
class LoadNPZ(Action):
    ...
    def run(self, filename=None):
        filename = filename or find_file(filt="NumPy ZIP file (*.npz)", parent=self.main)
        if filename:
            self.status.emit('reading %s' % (filename, ))
...
class SaveNPZ(Action):
    ...
    def run(self, filename=None):
        filename = filename or find_file(filt="NumPy ZIP file (*.npz)",
                                         parent=self.main, mode='save')
        if filename:
            ds = self.main.dataset
...
class RecentFileAction(Action):
    ...
    def run(self):
        ...
        if self._text.endswith('.set'):
            ...
            raise Exception('do not know how to load a %r file' % (self.filename.split('.')[-1], ))
