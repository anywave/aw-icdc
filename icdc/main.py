...
class TableWidget(pg.TableWidget):
    ...
    sort_col = 0
    sort_order = qt_core_enum('SortOrder', 'AscendingOrder') # QtCore.Qt.SortOrder.AscendingOrder
...
class ChannelTable(TableWidget):
    def __init__(self, *args, **kwds):
        ...
        self.contextMenu.addAction('Add channel(s) to picks').triggered.connect(self.add_picks)
        self.contextMenu.addAction('Remove channel(s) from picks').triggered.connect(self.rm_picks)
        self.contextMenu.addAction('Use selected channel(s) as picks').triggered.connect(self.use_picks)
        self.contextMenu.addAction("View channels' DWT").triggered.connect(self.pop_up_dwt)
    @property
    ...
    def summarize(self, ds):
        ...
        return np.array(
            zip(ds.labels, 
                stats.kurtosis(ds.data[:, ::(ds.data.shape[1]/1000 + 1)], axis=1),
                ds.nev_per_chan,
                ds.pick_mask
                ), 
            dtype=[
...
class EventTable(TableWidget):
    def summarize(self, ds):
        ...
        for id, e in enumerate(ds.events):
            ...
            if targets:
                if type(targets[0]) not in (str, unicode):
        ...
        return np.array(
            es,
            dtype=[('Id', int), ('Label', object), ('Time', float), ('Value', int), ('Target channels', object)])
    ...
    def update_summary(self, dataset):
        ...
        if ds.filename:
            ...
            self.lb_basic.setText(
                '%d chans x %d samples @ %.3f Hz' % (
                    ds.nchan, ds.nsamp, ds.fs))
            ncl = len(np.unique(getattr(ds, 'evlabels', [])))
            self.lb_event.setText('%d events, %d classes' % (len(ds.events), ncl))
            self.lb_preprocess.setText(unicode(ds.preprocess))
            self.lb_detect.setText(unicode(getattr(ds, 'detection', '')))
            self.lb_classify.setText(unicode(getattr(ds, 'classification', '')))
