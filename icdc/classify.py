...
class ExtractEvents(ParAction):
    ...
    _pars = [
        ...
        {'name': 'Window size', 'type': 'float', 'value': 0.4, 
            'limits': (0.0, 60.0), 'step': 0.05, 'suffix': 's', 'siPrefix': True},
    ]
...
class Cluster(ParAction):
    ...
    def run(self):
        cfg = ParamsDialog(parent=self.main, parameters=[
                {'name': 'Similarity dimension', 'type': 'int', 'value': 2, 'limits': (1, 3)},
                {'name': 'Neighborhood quantile', 'type': 'float', 'value': 0.05, 'limits': (0., 1.), 'step': 0.01},
                {'name': 'Min. n. class elements', 'type': 'int', 'value': 20, 'limits': (2, 1000000), 'step': 10},
            ])
        if cfg.exec_():
            ...
            ds.xi, ds.evlabels = cluster.spectral_dbscan(ds.ss, n_dim=sdim, eps=eps, min_samples=minel)
    ...
    def run(self):
        cfg = ParamsDialog(parent=self.main, parameters=[
                {'name': 'Window size (pre + post)', 
                    'type': 'float', 'value': 0.4, 'siPrefix': True, 'suffix': 's', 'step': 0.05},
                {'name': 'Similarity measure', 'type': 'list', 'values': ['corrcoef', 'cov'], 'value': 'corrcoef'},
                {'name': 'Use absolute value of measure', 'type': 'bool', 'value': False},
                {'name': 'Similarity dimension', 'type': 'int', 'value': 2, 'limits': (1, 3)},
                {'name': 'Neighborhood quantile', 'type': 'float', 'value': 0.05, 'limits': (0., 1.), 'step': 0.01},
                {'name': 'Min. n. class elements', 'type': 'int', 'value': 20, 'limits': (2, 1000000), 'step': 10},
            ])
        if cfg.exec_():
            ...
            ds.oob, ds.epochs = util.extract_windows(ds.fs, ys, peaks, pre=-winsz/2.0, post=winsz/2.0, echan=echan)
...
class ClassSpace(ParAction):
    ...
    def run(self):
        cfg = ParamsDialog(parent=self.main, parameters=[
            {'name': 'Window size (pre + post)', 
                'type': 'float', 'value': 0.4, 'siPrefix': True, 'suffix': 's', 'step': 0.05},
            {'name': 'Use all channels\n(use picked if unchecked)', 'type': 'bool', 'value': False},
            {'name': 'N. spatial components', 'type': 'int', 'value': 3, 'limits': (1, 1000)},
            {'name': 'Similarity dimension', 'type': 'int', 'value': 2, 'limits': (1, 3)},
            {'name': 'Neighborhood quantile', 'type': 'float', 'value': 0.05, 'limits': (0., 1.), 'step': 0.01},
            {'name': 'Min. n. class elements', 'type': 'int', 'value': 20, 'limits': (2, 1000000), 'step': 10},
        ])
        ...
        if ret:
            ...
            peaks = [e['time'] for e in ds.events]
            ds.oob, ds.epochs = util.extract_windows(ds.fs, ys, peaks, pre=-winsz/2.0, post=winsz/2.0)
            self.status.emit('windows extracted, computing similarity..')
            ds.us, ds.ss = cluster.subspace_similarity(ds.epochs, ncomp)
            self.status.emit('computing spectral embedding & clustering')
            ds.xi, ds.evlabels = cluster.spectral_dbscan(ds.ss, n_dim=sdim, eps=eps, min_samples=minsamp)
            for e, o, el in zip(ds.events, ds.oob, ds.evlabels):
                e['value'] = 4199 if o else 4201 + el
