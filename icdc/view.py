...
class ShowSignals(Action):
    ...
    def run(self):
        ...
        self.sigwin = compare(ds.fs, np.array(ds.labels)[ds.pick_mask], ds.data[ds.pick_mask])
...
class ClassAtlas(Action):
    ...
    def run(self):
        ...
        ca = atlas.ClassAtlas(ds.fs, events, ds.epochs, ds.xi, ds.evlabels, np.array(ds.labels)[ds.pick_mask], parent=self.main)
        ca.show()
...
actions = [ShowSignals, SignalCrossCorr, 'separator', ShowDetection, EventSimilarity, 
    ClassAtlas, ClassRates, 'separator', OpenDicom]
